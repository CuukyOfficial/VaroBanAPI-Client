...
public class BanUser {
	...
	public Ban[] getBans(AccountType type) {
		switch (type) {
		case MINECRAFT:
			return this.getMinecraftBans();
		case DISCORD:
			return this.getDiscordBans();
		default:
			throw new Error();
		}
	}

	public boolean hasActiveBan(AccountType type) {
		switch (type) {
		case MINECRAFT:
			return this.hasActiveMinecraftBan();
		case DISCORD:
			return this.hasActiveDiscordBan();
		default:
			throw new Error();
		}
	}

	public Ban getLatestBan(AccountType type) {
		switch (type) {
		case MINECRAFT:
			return this.getLatestMinecraftBan();
		case DISCORD:
			return this.getLatestDiscordBan();
		default:
			throw new Error();
		}
	}
